...
@pytest.mark.parametrize(
    ...
    [
        ...
        param(
            lambda t: t.string_col.cast(
                "double"
            ),  # https://docs.snowflake.com/en/sql-reference/data-types-numeric.html#double-double-precision-real
            lambda at: sa.cast(at.c.string_col, sa.DECIMAL),
            id="string_to_double",
        ),
        param(
...
@pytest.mark.parametrize(
    ...
    [
        ...
        param(lambda x, y: x.floordiv(y), L(7), 2, 3, id="floordiv_no_literal"),
        param(lambda x, y: x.rfloordiv(y), L(2), 7, 3, id="rfloordiv_no_literal"),
    ],
)
...
@pytest.mark.parametrize(
    ("expr", "expected"),
    [
        param(L("abcd").re_extract("([a-z]+)", 1), "abcd", id="re_extract_whole"),
        param(
            L("How are you doing today?").re_extract("\\b\\S*o\\S*\\b", 3),
            "you",
            id="re_extract_first",
        ),
