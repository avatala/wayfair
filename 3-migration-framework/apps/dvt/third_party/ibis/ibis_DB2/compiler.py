...
from ibis.backends.base_sqlalchemy.alchemy import (_get_sqla_table,
                                                   _variance_reduction,
                                                   fixed_arity, infix_op,
                                                   unary)
...
def _millisecond(t, expr):
    ...
    # mod 1000
    (sa_arg,) = map(t.translate, expr.op().args)
    return sa.cast(sa.func.floor(sa.extract("millisecond", sa_arg)), sa.SMALLINT) % 1000
