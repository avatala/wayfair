...
@pytest.mark.parametrize(
    ("func", "left", "right", "expected"),
    [
        param(operator.add, L(3).cast(dt.int16), L(4).cast(dt.int16), 7, id="add"),
...
@pytest.mark.parametrize("pattern", ["%Y%m%d", " %X", " %d ", " %I ", " %H%S ", " %p "])
def test_strftime(con, pattern):
    ...
    assert con.execute(final_value.strftime(pattern)) == raw_value.strftime(pattern)
