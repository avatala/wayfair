...
_token_names = dict(
    (getattr(dt.Tokens, n), n) for n in dir(dt.Tokens) if n.isalpha() and n.isupper()
)
...
class Number(dt.DataType):
    ...
    def __str__(self) -> str:
        return "{}({:d}, {:d})".format(self.name.lower(), self.precision, self.scale)
...
def can_cast_numbers(source: Number, target: Number, **kwargs) -> bool:
    return target.precision >= source.precision and target.scale >= source.scale
