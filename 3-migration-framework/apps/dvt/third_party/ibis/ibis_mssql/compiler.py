...
from ibis.backends.base_sqlalchemy.alchemy import (_cumulative_to_reduction,
                                                   fixed_arity, unary)
...
def _log(t, expr):
    ...
    if base is not None:
        ...
        return sa.cast(
            sa.func.log(sa.cast(sa_arg, sa.NUMERIC), sa.cast(sa_base, sa.NUMERIC)),
...
def _day_of_week_index(t, expr):
    ...
    return sa.cast(sa.cast(sa.extract("dow", sa_arg) + 5, sa.SMALLINT) % 7, sa.SMALLINT)
