...
from data_validation.result_handlers.stackdriver import \
    StackdriverResultHandler
...
class ConfigManager(object):
    ...
    def get_result_handler(self):
        ...
        elif result_type == "Stackdriver":
            if self.config[consts.CONFIG_TYPE] == consts.SCHEMA_VALIDATION:
                cols_filter_list = consts.SCHEMA_VALIDATION_COLUMN_FILTER_LIST
            else:
                cols_filter_list = consts.COLUMN_FILTER_LIST

            source_database = self.config[consts.CONFIG_SOURCE_CONN]["database"]

            print("database: {}".format(source_database))

            return StackdriverResultHandler(
                self._config.get(consts.CONFIG_FORMAT, "table"),
                self.filter_status,
                cols_filter_list,
                project_id=self.result_handler_config[consts.PROJECT_ID],
                source_database=source_database,
                environment=self.result_handler_config[consts.ENVIRONMENT],
            )
        else:
